...
def check_and_retry_pending_cases(connection):
    ...
    try: 
        ...
        conn = get_connection()
        if conn is None:
            logger.error("[CREATE CASE] ⚠ DB connection failed. Will retry later")
            return []
...
def schedule_pending_retry(connection):
    """Retry thread — wakes up when a pending case is added or every 60s."""
    global IS_RECIEVING_DATA
    while True:
        try:
            triggered = PENDING_CASE_EVENT.wait(timeout=30)  # Wait for event or timeout
            
            if triggered:
                # Event किसी और thread/process ने set किया है
                logger.info("Pending case event triggered early — retrying immediately.")
            else:
                # Timeout पूरा हो गया, event trigger नहीं हुआ
                logger.info("No event trigger, woke up due to 90s timeout — retrying now.")
                
            PENDING_CASE_EVENT.clear()
