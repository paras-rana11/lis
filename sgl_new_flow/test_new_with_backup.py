...
from healper import GenerateChecksum, MachineConnectionSerial, ConnectionDb
...
ERROR_LOG_FILE = 'C:\\ASTM\\root\\log_file\\test_error_logging_for_c_311_2.log'
...
print("establish just call class ")            
def get_connection():
    try:
        db = ConnectionDb(
            host="localhost",
...
def schedule_pending_retry(connection, interval_sec=120):
    """
        ...
        try:
            if MONITORING_ACTIVE :
                try:
                    ...
                    byte = connection.read()
                    logger.info(f"MONITORING_ACTIVE Reading byte: {byte}")
                    
                    if byte and byte != b'' and byte == '\x05':
                        logger.info("Received ENQ from machine")
                        with FLAG_LOCK:
                ...
                with FLAG_LOCK:
                    paused = IS_PENDING_RETRY_RUNNING
                if paused:
...
def start_monitoring():
    ...
    try:
        ...
        retry_thread = threading.Thread(target=schedule_pending_retry,args=(connection_c_311_2, 60), daemon=True,name="PendingRetryThread")
        retry_thread.start()
        logger.info("Pending retry thread started.\n")
...
if __name__ == "__main__":
    try:
        ...
        # threading.Thread(target=cleanup_old_backup_from_db, args=(86400, 10), daemon=True, name="clean_old_backup_from_db").start()
